...
if ($result->num_rows > 0) {
  while($row = $result->fetch_assoc()) {
      ...
      $solution_ex = $row["solFF"]; $temps_ex = $row["tempsFF"];
...
$sql = "UPDATE resultats SET `poids_moyen`='$poids_moyen',`capacite`='$capacite',`nombre_objets`='$nombre_objets',`solFF`='$solFF',`tempsFF`= '$tempsFF' WHERE id='$id'";
    ...
    // classe U ou T
    $sql = "UPDATE resultats SET `capacite`='$capacite',`nombre_objets`='$nombre_objets',`solFF`='$solFF',`tempsFF`= '$tempsFF' WHERE id='$id'";
  }
